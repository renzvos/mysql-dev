...
if [ $(id -u) = "0" ]; then
	echo "[Entrypoint] Entrypoint and container running"
	is_root=1
	install_devnull="install /dev/null -m0600 -omysql -gmysql"
	MYSQLD_USER=mysql
else
	echo "[Entrypoint] Entrypoint and container not running"
	install_devnull="install /dev/null -m0600"
	MYSQLD_USER=$(id -u)
fi

echo "[Entrypoint] Contacting MYSQL"
if [ "$1" = 'mysqld' ]; then
	...
	echo "[Entrypoint] MYSQL is running"
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		echo "[Entrypoint] Found MYSQL Datadir"
